...
public class GM_cuerpo : MonoBehaviour
{
    ...
    public GameObject Game_Manager_cuerpo;
    ...
    void Awake()
    {
        firebaseManager = Game_Manager_cuerpo.GetComponent<FirebaseManager>();
    }
    ...
    void Start()
    {
        ...
        hints.Add("Órgano del cuerpo con el qué podemos ver nuestro entorno"); //ojo
        hints.Add("Parte inferior del cuerpo que nos permite caminar y correr"); //pie
        hints.Add("Órgano con dos orificios que permite respirar"); //nariz
        hints.Add("Parte del cuerpo con dedos que está al final del brazo y nos permite realizar muchas tareas"); //mano
        hints.Add("Parte del cuerpo que utilizamos para hablar, comer y respirar"); //boca
        hints.Add("Parte del cuerpo que se encuentra en la mano o en el pie y permite realizar actividades precisas"); //dedo
        hints.Add("Órgano que permite percibir los sonidos del entorno"); //oreja
        hints.Add("Conjunto de pelos de la cabeza"); //cabello
        hints.Add("Parte del cuerpo que va desde el hombro hasta la mano y nos permite realizar movimientos"); //brazo
        hints.Add("Parte superior del cuerpo humano donde está el cerebro"); //cabeza

        instructions.Add("¿Ahora podrías ayudarme a saber las partes del cuerpo de un ser humano?");
        Tutorial(0);
    ...
    public void OnFinalResult(string result)
    {
        ...
        if (i == (lengthImages - 1) & isGameActive & timerOn)
        {
            if (images[i].CompareTag(result))
            {
                ...
                UpdateTimer(timeLeft);
                images[i].SetActive(false);
                StartCoroutine(ShowAndWait(1.5f));
