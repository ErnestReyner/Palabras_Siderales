...
public class GM_relacionar_platos : MonoBehaviour
{
    ...
    public GameObject Game_Manager_platos;
    ...
    void Awake()
    {
        firebaseManager = Game_Manager_platos.GetComponent<FirebaseManager>();
    }
    ...
    void Start()
    {
        ...
        hints.Add("Relaciona dos imágenes de cocina y comida y dilo como este ejemplo: 'botella Y leche'");
        instructions.Add("Tengo mucha hambre... ¿Puedes ayudarme a escoger correctamente cada alimento con" +
            " su utensilio?");
        Tutorial(0);
    ...
    public void Success()
    {
        ...
        finalScoreText.text = " * Puntuación final: " + score + "/6";
    ...
    public void OnFinalResult(string result)
    {
        ...
        if (i == (lengthImages - 2) & isGameActive & timerOn)
        {
        ...
        if (i < (lengthImages - 2) & isGameActive & timerOn)
        {
            if (images[i].CompareTag(result))
            {
                ...
                StartCoroutine(ShowAndHide(images[i+1], 2.0f));
                StartCoroutine(ShowAndHide(correctSign, 2.0f));
                StartCoroutine(ShowAndHide(resultText.gameObject, 2.0f));
                StartCoroutine(ShowAndHide(currentWordText.gameObject, 2.0f));
                StartCoroutine(NextGO(images[i + 2], 2.0f));
                StartCoroutine(NextGO(images[i + 3], 2.0f));
                UpdateScore(1);
                UpdateTimer(timeLeft);
                wrongTimes = 0;
                i += 2;
